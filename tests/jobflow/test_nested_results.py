...
def test_nested_bulk_slabs_results(tmp_path):
    ...
    with change_settings({"NESTED_RESULTS": True, "RESULTS_DIR": tmp_path}):
        ...
        matches = list(
            tmp_path.glob(
                "bulk_to_slabs_flow*/bulk_to_slabs_subflow*/relax_job-*/opt.log.gz"
            )
        )
        assert len(matches) == 4


def test_nested_results(tmp_path):
    with change_settings({"NESTED_RESULTS": True, "RESULTS_DIR": tmp_path}):
...
def test_nested_results_flow_replace(tmp_path):
    with change_settings({"NESTED_RESULTS": True, "RESULTS_DIR": tmp_path}):
        from quacc import flow, job
        ...
        assert any(
            tmp_path.glob(
                "create_file_flow*/create_file_job*/create_file_job_replaced*/foo"
            )
        )
